...
def proceed_to_booking():
    ...
    for i in range(len(available)):
            if available[i].seat_no ==temp_seat:
                Label(seat_booking_screen,text=available[i].seat_no,bg="blue").pack()
            else:
...
def payBooking():
    ...
    final = cinema.payDues(choice,card_store.get())
    exit_payment_screen()
    book_tickets()

def book_tickets():
    global book_screen
    book_screen=Toplevel(main_screen)
    book_screen.geometry("600x400")
    Label(book_screen,text="TOTAL BOOKED TICKETS",font=("Calibri", 20,'bold')).pack()
    Label(book_screen,text="").pack() 
    for i in range(len(final)):            
        Label(book_screen,text="Movie >"+final[i].show.movie.name+"| Hall> "+final[i].show.hall.hall_no + " | Time>"+ final[i].show.time_slot+ " | Seat > " + final[i].seat.seat_no ,font=("Calibri", 10,'bold')).pack()
        Label(book_screen,text="").pack()            
...
def pay_due_membership():
    ...
    m_ammount = cinema.initiateMemberDues()
    
    monthly_bill = str(m_ammount)
    ...
    if m_ammount != -1:
        ...
        Label(pay_dues_Screen,text="Member Dues = " + monthly_bill).pack()
        Label(pay_dues_Screen,text="").pack()
        ...
        nms = Toplevel(main_screen)
        nms.title("Not Member")
        nms.geometry("400x400")
        Label(nms,text="").pack()
        Label(nms,text="").pack()
        Label(nms,text="").pack()
        Label(nms,text="NOT MEMBER",bg="red").pack()
...
def Delete_Account_Success():  # add functionality of delete account here 
    ...
    Label(Delete_Account_Screen,text="ACCOUNT DELETION SUCCESS").pack()
    Label(Delete_Account_Screen,text="").pack()
...
def Start_Up_Interface():
    ...
    global main_screen
    main_screen = Tk()
    main_screen.geometry("1000x450")
    main_screen.title("CINE-GOLD")
    main_screen.configure(background='orange')
  
    Label(text="Select Your Choice", bg="purple",foreground='white', width="400", height="2", font=("Calibri", 13,'bold')).pack()
    Label(text="").pack()
